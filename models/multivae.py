...
class MultiVAE(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.lat_dim = config['embedding_size']
        ...
        self.rows = torch.tensor(interaction_matrix.row.astype(np.int64)).to(self.device)
        self.cols = torch.tensor(interaction_matrix.col.astype(np.int64)).to(self.device)
    ...
    def get_rating_matrix(self, user):
        ...
        col_indices = self.pos_items[user].flatten()
        row_indices = torch.arange(user.shape[0]).to(self.device).repeat_interleave(self.pos_items.shape[1], dim=0)
        indices_mask = self.pos_items_mask[user].flatten().type(torch.BoolTensor)
        # mask out
        ...
        rating_matrix.index_put_((row_indices, col_indices), torch.ones(row_indices.shape[0]).to(self.device))
